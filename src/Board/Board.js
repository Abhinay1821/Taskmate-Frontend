  ...
  const Board = () => {
    ...
    const handleSaveEdit = (updatedTask) => {
      ...
      setInProgressTasks(updatedTasks.filter(t => t.status === 'inProgress'));
    ...
    const handleDeleteTask = async (status, taskId) => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          else if (status === 'inProgress') setInProgressTasks(updatedTaskForStatus);
    ...
    const handleDragEnd = (result) => {
        ...
        if (source.droppableId === destination.droppableId) {
            ...
            else if (source.droppableId === 'inProgress') setInProgressTasks(updatedList);
            ...
            else if (source.droppableId === 'inProgress') setInProgressTasks(startUpdated);
            else setCompletedTasks(startUpdated);
    
            if (destination.droppableId === 'todo') setTodoTasks(finishUpdated);
            else if (destination.droppableId === 'inProgress') setInProgressTasks(finishUpdated);
    ...
    return (
      <div className="board-container">
            ...
            <Droppable droppableId="inProgress">
              {(provided) => (
                <div className="board-column" ref={provided.innerRef} {...provided.droppableProps}>
                  ...
                  <div className="tasks">{renderTasks(inProgressTasks, 'inProgress')}</div>
                  {provided.placeholder}
                </div>
              )}
            </Droppable>
        ...
        {/* Conditionally render the EditTaskModal when a task is being edited */}
        {editTask && (
          <EditTaskForm
            boardId={id}
            task={editTask}
            onSave={handleSaveEdit}
            onClose={() => setEditTask(null)} // Close the modal when done
          />
        )}
      </div>
    );
